...
const SOL_ACTION = 'solana-action:';
...
const isValidElement = (elem: Element): Element | null => {
    const aLink = elem.querySelector('a');
    return aLink;
};

const extractMetadata = (res: string) => {
    const startIdx = res.indexOf(SOL_ACTION);
    if (startIdx < 0) {
        return;
    }
    const endIdx = res.slice(startIdx).indexOf('"');
    if (endIdx < 0) return;
    return res.slice(startIdx, startIdx + endIdx);
};

const getActionUrl = async (url: string) => {
    try {
        const res = await fetch(url);
        const resJson = await res.text();
        const actionUrl = extractMetadata(resJson);
        return actionUrl;
    } catch (err: any) {
        console.error(err);
    }
...
const handleElement = async (elem: Element, parent: Element) => {
    const link = elem.getAttribute('href');
    if (!link) return;
    const actionUrl = await getActionUrl(link);
    if (!actionUrl) return;
    try {
        const url = actionUrl.split(SOL_ACTION)[1];
        ...
        // already unfurled
        if (elem.parentNode?.querySelector(`#${newId}`)) return;

        console.log(`unfurl: ${url}`);

        (parent as HTMLElement).style.border = 'none';
        renderActions(url, elem);
        // element.remove()
    } catch (error) {
...
const searchLinkCard = async () => {
    const elems = document.querySelectorAll('div[data-testid="card.wrapper"]');
    for (const elem of elems) {
        const aTag = isValidElement(elem);
        if (aTag) {
            await handleElement(aTag, elem);
        }
    }
};
...
const fetchTest = async () => {
    console.log('fetch------------')
    try {
        const res = await fetch('https://t.co/VepaaM2mBD');
        const resJson = await res.text()
        console.log({ resJson })
    } catch (err) {
        console.error(err)
